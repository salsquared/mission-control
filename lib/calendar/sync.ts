...
export async function syncEventToGcal(
    ...
    // Multi-user owner/crew (docs/multi-user-crew.html OQ6a): crew have no
    // Google grant by design, so they have no `Account` row at all. Without
    // this check every crew-created event falls into the catch below and writes
    // a warn into the 500-deep log ring buffer the OWNER reads — noise about a
    // non-event, generated by other people's normal use. "No linked Google
    // account" is an expected state here, not a failure, so detect it up front
    // and return quietly. A genuine auth failure for a user who DOES have an
    // account still warns, because that one is real and the owner wants to see
    // it. Same defect class as the crew gcal-poll flood closed in P3.7.
    ...
    if (!hasGoogleAccount) {
        if (opts?.throwOnError) {
            // Wording deliberately matches lib/googleapis.ts:12's message —
            // callers and smokes classify this failure by "not linked", and a
            // second phrasing for the same condition splits that taxonomy.
            throw new Error(`User Google account not linked (no Account row) — user ${userId}`);
        }
        return null;
    }
