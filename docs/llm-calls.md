...
| `lib/profile/bullet-assist.ts` (callBulletAssist) | `MODEL_LITE` (`gemini-3.1-flash-lite`) | 4096 (fill) / 2048 (rewrite) | ~8 KB user prompt (capped inside builder) | M7.6 / S7.7+S7.8+S7.9 — bullet drafting + rewriting + archive grounding on Profile entries. Originally pinned `gemini-3.1-flash` literal per user direction; that SKU doesn't exist (Google only ships the `-lite` variant at the 3.1 tier — 404 on first live call 2026-05-24). Swapped to `MODEL_LITE` since the user reviews every output (Accept/Discard on rewrite, edit-before-save on fill). Rate-limited at `profile:bullet-assist` 20 / 10 min in the route. |
