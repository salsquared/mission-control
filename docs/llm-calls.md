...
| `lib/resumes/tagline-tailor.ts` (tailorResumeTagline) | `resume-tagline` | `MODEL_LITE` (`gemini-3.1-flash-lite`) | 256 | profile summary (~4 KB capped) + parsed posting | ✅ | Posting-tailored subtitle under the resume H1. One call per generate. Sibling of `tagline-draft`; that one is posting-agnostic and runs from the Profile dash, this one runs at resume-gen time and grounds on profile + posting so the user can apply to roles outside their default identity (e.g. security guard while the profile tagline reads as a SWE pitch). Best-effort — AIError surfaces a warn and resume gen falls back to `profile.tagline`. Persisted on `GeneratedResume.tagline`. Reuses `postFilterTagline` from `lib/profile/tagline-draft.ts` for the same one-sentence / ≤200ch / trailing-period invariants. |
