...
3. **Manual UI smoke** in `npm run dev` (port 4101): open Profile dash → create work role → add bullets → toggle locked/excluded → reorder → reload. Second tab to confirm `'Profile'` SSE cross-tab sync. Confirm or change `viewHue: 280`.
4. **Commit M7 UI.** One commit: `feat(profile): add Profile dash UI`. Bundle cards + view + Dashboard wiring + state wiring + hook union + bullets utility refactor + child-route lint cleanup. No AI attribution per the `feedback_no_commit_attribution` memory.
5. **Start M8 design.** Read user stories §8 (34, 38 are the 🔴) + Decision 3 (HTML → headless-Chromium print to PDF). Write a milestone-level plan into this doc under "Immediate next actions" before coding.
