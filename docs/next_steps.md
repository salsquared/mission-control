...
1. **Commit + push the LOP-6 cutover.** Bundle the uncommitted tree (new `scripts/sync-lunary-templates.ts`, new `lib/ai/prompts.ts`, 8 callsite cutovers, async builder ripple in smoke + route + eval/provider.ts, doc updates) + the older `0581845` + `d9ecd07`. Pre-push gate already green for the working tree.
2. **End-to-end Lunary trace check.** Trigger any LLM path through the running dev process (e.g. fill some bullets on the Profile dash, paste a posting URL) and confirm runs appear in Lunary's dashboard tagged by their `name` (one of: `bullet-assist-fill`, `bullet-assist-rewrite`, `resume-rewrite`, `posting-parse`, `profile-import`, `profile-synthesize`, `discovery-suggest`, `employment-type-classifier`, `email-parser`). PM2 dev tier is already restarted with the env vars loaded; this is just a "does the round-trip work" smoke.
3. **Run `npm run test:prompts` once end-to-end.** Validates the Promptfoo harness against all 9 cut-over callsites with synthetic seed fixtures. Expected ~$0.01–0.05 in Gemini spend. First-run failures are the cue to capture real fixtures (LOP-9) and tighten assertions — they're not a regression of this session's work.
4. **LOP-9 real fixture captures (still TODO).** Add a `CAPTURE_FIXTURES=1` env gate to the start of `chatJSON` that emits `console.info('[FIXTURE]', JSON.stringify({ name, system, user }))`. Then `CAPTURE_FIXTURES=1 pm2 restart mission-control-dev --update-env`, use the app for ~30 min covering the flows you care about, grep the pm2 logs for `[FIXTURE]`, and replace the synthetic-but-realistic seed entries in `eval/suites/*.yaml` with the real captures.
