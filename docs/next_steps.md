...
- **Branch:** `main`. **Four things landed end-to-end, four commits ahead of `origin/main`:**
  ...
  - (next) — Story 48 resume-version diff (no schema; new lib + route + inline diff panel).
...
1. **Story 63 — bulk-move applications between tracks (🔵).** Single-row Track toggle shipped in MB Phase 4; bulk-select UI ("reclassify N apps at once") still open. Multi-select on kanban cards + a track-flip action.
2. **Story 24 — compensation parsing (🔵).** Regex over `JobPosting.snippet` → `compensationRangeMin/Max` columns; surfaces salary ranges in the postings card. Cheap fetcher-side win.
3. **Story 46 — README ingestion (🔵).** Extend the M9 GitHub-metrics pipeline to pull READMEs from `portfolio=true` repos and feed them into the bullet-rewrite prompt as additional source material.
4. **Story 45 — suggested portfolio rewrites (🔵).** Detect metric deltas (star threshold, new language, big release) in the GitHub-metrics scheduler and enqueue a `Notification(kind='system')` suggesting the user revisit those project bullets.
...
7. **RAH-12 — per-userId Gemini rate-limit (🟡 abuse).** Token bucket on `POST /api/resumes` + `POST /api/profile/import` (e.g. 5 generations / 10 min) checked before the first Gemini call. Single-user today, but a logged-in tab in a loop drains the free-tier quota.
