...
- **Post-fix-4-6 worker baseline** (5 min idle, 63 samples): worker RSS median **1071 MB (−15 %)**, p95 1216 MB (−15 %), peak **1217 MB (−17 %)**, CPU max **7.3 % (−50 %)**. Smoother glide (1216 → 947 MB), no big GC pause during the window. The ~950 MB steady-state floor is webpack HMR / Next-dev internal caches, not our app — would need Turbopack or scheduled worker recycle to address.
- **Net of the session**: peak RSS 1.46 GB → 1.22 GB (~16 % drop), 100 % CPU peg → gone, smoother allocation curve. Safari "page reloaded" symptom should be much less frequent. Further wins are in Next-dev internals, not in our code.
