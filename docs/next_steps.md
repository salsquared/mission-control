...
- **Branch:** `main`. **Seven things landed end-to-end, seven commits ahead of `origin/main`:**
  ...
  - (next) — Story 46 README ingestion (`Project.readme` + scheduler fetch + rewrite-prompt context).
...
1. **Story 45 — suggested portfolio rewrites (🔵).** Detect metric deltas (star threshold, new language, big release) in the GitHub-metrics scheduler and enqueue a `Notification(kind='system')` suggesting the user revisit those project bullets.
2. **Story 33 rollback UX (🔵).** Capture side shipped; restore-from-snapshot is the deferred half. Build a destructive-overwrite confirm + transactional bulk-replace of `WorkRole` / `Project` / `Education` from the stored payload. Defer until the user actually wants to roll back.
