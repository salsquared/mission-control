...
- **Honest post-fix worker measurement** (one-off, dev up ~3.3 h): worker RSS **~700 MB idle**, CPU 0–1.6 %. That's ~38 % below the 1.13 GB pre-fix baseline, and the 100 % CPU peg is gone, but it's still high. User confirmed "more stable" qualitatively. **Remaining queued fixes (4 memoize InternalView, 5 scoped cache-invalidation, 6 cache pingDatabase) are load-bearing — not optional.** Fresh long-run measurement after each remaining fix is needed.
