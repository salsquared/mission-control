...
- **Date:** 2026-05-18
- **Branch:** `main` (uncommitted — added Tier-B LLM employment-type classifier on top of yesterday's PA/PB-ext/PC sweep). `lib/ai/classify-employment-type.ts` batches up to 50 postings per Gemini Flash call, gated to brand-new postings only by `scheduler/jobs/job-watcher.ts` (one bulk findMany now replaces the old per-posting findUnique). Live smoke at `scripts/tests/employment-type-classifier-live.ts` runs 11 curated fixtures; first two runs measured 41s and 18s wall-time. Timing logged via `[employment-type-classifier]` console.info lines so it surfaces in the in-app log viewer. Per the user's plan: if those numbers consistently exceed ~15s/batch in production, swap `classifyOneBatch` from `chatJSON` to a local Gemma-3n call on the mac mini.
