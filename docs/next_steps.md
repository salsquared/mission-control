...
- **Branch:** `main`. **Five things landed end-to-end, five commits ahead of `origin/main`:**
  ...
  - (next) — Story 63 bulk-move applications between tracks (no schema; new route + select-mode UI).
...
1. **Story 24 — compensation parsing (🔵).** Regex over `JobPosting.snippet` → `compensationRangeMin/Max` columns; surfaces salary ranges in the postings card. Cheap fetcher-side win.
2. **Story 46 — README ingestion (🔵).** Extend the M9 GitHub-metrics pipeline to pull READMEs from `portfolio=true` repos and feed them into the bullet-rewrite prompt as additional source material.
3. **Story 45 — suggested portfolio rewrites (🔵).** Detect metric deltas (star threshold, new language, big release) in the GitHub-metrics scheduler and enqueue a `Notification(kind='system')` suggesting the user revisit those project bullets.
