...
- **S7.10** 🟡 As a user editing a bullet, I want the existing per-bullet "AI rewrite" affordance (the wand icon from [[S7.8]]) to update *only the bullet text* — and a *separate* per-bullet "AI tag" affordance to propose tags for that bullet without touching the text. Today's shipped M7.6 behavior conflates two intents: polishing wording vs reconsidering skill labels. A user who just wants to clean up a sentence shouldn't also have to accept tag churn, and a user whose sentence is already strong but whose tags are stale or missing shouldn't have to rewrite the whole bullet to fix them. This narrows the [[S7.8]] rewrite scope (text only — reverts the post-launch tag-update enhancement) and adds a sibling button for on-demand tag regeneration. Both affordances feed back into the existing PATCH flow; nothing else about merge / accept-discard / locked-bullet semantics changes. The tag affordance grounds on the bullet's own text plus the profile's existing tag vocabulary (for consistency) — no posting context required — and is distinct from the bulk auto-tag pass at resume-generate time ([[S8.9]]). **Tag-density bounds:** the generator targets **3 to 7 tags per bullet** — it should aim for at least 3 even on terse bullets and never propose a result that pushes the bullet over 7. **Cap guard:** if the bullet already has ≥ 7 tags, the affordance short-circuits before the LLM is called (no token spend, no proposal) and surfaces a "tag limit reached — remove or unpin a tag first" hint. Floor is soft (LLM may return fewer than 3 when there's genuinely no defensible label to propose); ceiling is hard.
- **S7.11** 🟡 As a user with tags I've deliberately curated (e.g. `Python`, `distributed-systems`, `leadership`), I want a per-tag "pin" toggle so the per-bullet AI-tag generator from [[S7.10]] *and* the bulk auto-tag pass from [[S8.9]] both leave my pinned tags alone — pinned tags are never auto-cleared, never re-suggested as duplicates, never overwritten by either flow. A pinned tag persists across regenerates until I explicitly unpin (or remove) it. Schema: per-bullet `pinnedTags: string[]`, where every entry is also in `tags`. Visual: a small pin glyph inside the chip when pinned. Invariants: pinning a tag also in `autoTags` is fine (auto + pinned = "auto-suggested but locked in"); a tag in `removedTags` cannot be pinned — the blocklist wins. **LLM awareness:** when either the per-bullet generator ([[S7.10]]) or the bulk pass ([[S8.9]]) runs, the prompt receives both lists explicitly — pinned tags as "anchors, must remain in the output unchanged" and unpinned tags as "candidates: keep, replace, or remove to make room for better proposals." This lets the LLM treat pins as the immutable spine and churn only the rest, while still honoring [[S7.10]]'s 3–7 cap counting pinned + unpinned together.
- **S7.12** 🔵 As a user, clicking a tag chip's body should do nothing — currently (post-M8.5.6) a chip-body click removes the tag *and* adds it to the per-bullet blocklist ([[S8.9]] `removedTags`), which is a footgun: a hover-misclick on a busy bullet silently erases work. Tag removal moves to an explicit per-chip delete affordance (the X icon) so the intent is unambiguous, and the X gets ~1px larger for a more comfortable click/tap target. The blocklist semantics stay identical — only the trigger surface changes.
- **S7.13** 🟡 As a user filling out my profile, I want a **per-entity** free-form "scratchpad" — one on each WorkRole / Project / Education row — where I can write a paragraph or two in my own words about that specific role: what I actually did, the problems I solved, the tools I reached for, how I'd describe the work to a friend over coffee. The scratchpad is **voice + experience grounding** for the LLM, not a structured field that ever ends up on a resume verbatim. It complements (does not replace) the entity's structured bullets — it's an additional substrate the LLM consults when generating or rewriting bullets for that specific entity.
  - **Schema:** new `scratchpad: String?` on `WorkRole`, `Project`, *and* `Education` (three columns, one per entity table), each capped at ~8 KB to keep prompt budgets bounded.
  - **UI — hidden by default, overlay editor:** each entity row gets a small "Notes" / "Scratchpad" trigger button (icon, lucide `StickyNote` or similar). Inline the textarea is **hidden** — the row stays visually clean. Click the trigger → opens a modal overlay with a large multi-line textarea, save / cancel buttons, character count, and a placeholder like *"In your own words: what did you build at this role, what problems did you solve, what was hard, what energizes you?"* Save persists via the existing entity PATCH path; cancel discards. The trigger button's visual state reflects emptiness — outlined/muted when no scratchpad content, filled/colored (with a small dot or count) when populated, so the user can tell at a glance which entities have notes without opening each overlay. Optional — every entity can have an empty scratchpad and behaves exactly as today.
  - **Bullet-assist grounding** ([[S7.7]] fill + [[S7.8]] rewrite + [[S7.10]] tag): the entity's own scratchpad joins the existing grounding stack (spine, sibling bullets with tag overlap, [[S7.9]] archive spans, README) as an additional source — the LLM sees the user's raw voice for *that role specifically* alongside the structured material, so generated bullets read in the user's actual cadence rather than generic resume-speak. Cross-entity scratchpad bleed is avoided — fill/rewrite for WorkRole X only sees X's scratchpad, never Y's.
  - **Resume-gen synthesis** ([[S8.1]] tailored generation in `GenerateResumeCard.tsx`): on every generate, the pipeline reads the full structured profile (WorkRole / Project / Education with their bullets) *plus* each entity's scratchpad. A new "synthesize from scratchpad" pass runs after the existing select step — for each entity whose current bullets under-cover the posting keywords AND whose scratchpad is non-empty, the pass takes `entity.scratchpad + posting keywords + entity spine` and produces *fresh* bullet candidates (not just rewrites of existing). The selector mixes synthesized candidates with existing profile bullets and picks the strongest evidence per posting keyword, using the posting's terminology where the scratchpad supports it. Synthesized bullets show in the "Why these bullets?" trace ([[S8.2]]) with their own source label (e.g. `kind: "scratchpad-synth"`) so the user can see which came from the scratchpad pass vs the existing profile.
  - **No-fabrication invariant** ([[S8.9]]): the LLM cannot invent metrics, dates, or claims that aren't either in the entity's scratchpad or in the posting. Scratchpad is grounding evidence, not creative license. The scratchpad itself is **never** rendered into a resume verbatim — it's input to the LLM, not output to the document.
- **S7.14** 🟡 As a user, I want a separate one-sentence **tagline** field on my profile — distinct from my name — that I can either write myself or have the LLM draft for me, and that renders as a subtitle under my name on every generated resume. Today the `profile.headline` field doubles as both the name (what the resume's H1 prints, per the M8.4 canonical-naming work) and any tagline-y content the user happens to put there; this story splits the concerns. **Name** is plain text (e.g. "Salvador Salcedo"). **Tagline** is a one-line pitch of who I am professionally (e.g. "Software engineer focused on developer-facing systems and reliability"). Both render on every generated resume.
  - **Schema:** new `tagline: String?` on `Profile`, capped at ~200 chars at the zod layer. Backing schema field for the name stays `profile.headline` (legacy column name; renaming it would cascade through the renderer, the M8.4 canonical-naming helpers, every Promptfoo fixture, and a Prisma migration) — the UI label is the cheap fix and lands alongside this story (`PersonalInfoCard.tsx`: "Headline" → "Name").
  - **UI:** new Tagline field below the Name field on `PersonalInfoCard`. Plain text input + an **AI-draft** button (purple `Sparkles` icon matching the card theme). Click → small inline diff panel shows current → proposed text with Accept / Discard buttons. Same UX pattern as M7.6.9's bullet-rewrite diff.
  - **LLM behavior:** new `tagline-draft` callsite on `MODEL_LITE`. **Grounding is the full hydrated profile** — every WorkRole + Project + Education + Summary + Skills + Hobbies + Languages + every entity's scratchpad ([[S7.13]]) — passed in as a compact summary. Two modes, dispatched by whether the current tagline is empty:
    - **Empty tagline → draft from scratch.** The LLM produces a one-sentence tagline grounded ONLY on the profile's existing evidence. The output is a self-contained pitch the user can keep verbatim or edit.
    - **Non-empty tagline → enhance.** The LLM takes the existing text as the user's starting intent and refines it for fit + voice consistency with the rest of the profile. The user's angle is the floor — the rewrite preserves their framing, doesn't pivot to a different concept.
  - **Output rules:** one sentence, ≤ 200 chars (typically ≤ 120), no first-person pronouns or possessives ("I" / "my" / "me"), no trailing newlines, ends with a period. Should read as a professional one-liner, not a sales-y headline.
  - **No-fabrication invariant** ([[S8.9]]): the LLM cannot claim experience the profile doesn't already evidence. "10 years of Rust" only if the profile actually demonstrates 10 years of Rust. Conservative over aggressive.
  - **Resume template render:** `lib/resumes/templates/ats-plain.tsx` reads `profile.tagline` and renders it as a subtitle line directly under the name H1. Empty tagline = no subtitle line.
...
> **⚠️ Superseded — kept as historical record.** This in-app passcode-gate design was **superseded by Cloudflare Access at the edge** (design: [`cloudflare-access-auth.html`](./cloudflare-access-auth.html)). The access gate moved to the **Cloudflare edge** — an email-allowlisted Zero Trust app in front of both public hostnames — instead of a new application-level passcode. Google OAuth was reframed to **token-source-only**, exactly as these stories envisioned (S15.15–S15.19), but via the edge rather than an in-app lock. The S15.x stories below are retained unchanged as the original design record; do not implement them.
