...
**TL;DR — every designed milestone phase is now shipped; the open backlog is the audit-found soft-bug list + deferred S7.6 rollback UX + three M8.7 follow-ups.** Recent milestone landings (most-recent first): **M8.7** (resume relevance pipeline — importance weighting + section/entity ordering + posting-conditional pin, 2026-05-26) — commit `0e8085a`. **M7.7 + M7.8 + M8.6** wave (per-bullet tag UX refactor + per-entity scratchpad + resume-gen scratchpad synthesis, 2026-05-26). **M7.9** (profile tagline + LLM draft, 2026-05-26). **M7.6** (LLM bullet assist + resume-upload archive, 2026-05-23). **M8.4 + M8.5** (resume card v2 UX + LLM keyword coverage, 2026-05-25). Also 2026-05-25: [Close-detection probe gate](./archive/close-detection-probe.md) across all 11 ATS kinds — fixes false-closures (LinkedIn 24h filter, Workday 200-per-crawl page cap) and recovered 921 dev + 629 prod erroneously-closed postings. **Forward motion now runs through real-world use** (apply, observe LLM rewrites, iterate prompts via the Promptfoo harness) plus the parked audit-bug list + M8.7-followups in `docs/next_steps.md`. **Deferred-by-design**: **Story S7.6** restore-from-snapshot UX, parked until the read-only safety net proves useful enough to justify a destructive path. **M8.7 follow-ups** (logged): Profile UI affordance for pin-editing (currently CLI-only), commodity-only entity penalty (scoring change #3), pin-priority ordering among multiple pinned entities in the same section.
...
| C | M8.7 — Resume relevance: ordering, weighting, pinning | ✅ | Shipped 2026-05-26 (commit `0e8085a`), 7 sub-features in one wave. `posting-parse` per-keyword `importance(1-5)` weighting + `scoreBullet` importance multiplier + case-insensitive matchedTags dedup. `resume-rewrite` pre-filter (no-match bullets pass through). Skills/Languages/Interests rendered + posting-filtered. `resume-tagline` extended with `sectionOrder` + `entityOrder` + evidence-rich entity-IDs block. `pinKeywords` schema column (migration `20260526220116_add_pin_keywords_to_entities`) + selector keep-always + one-page-pruner unremovable + LLM-reorder pin-front. Tag case-sensitivity bug class fixed across 3 sites + one-shot backfill ran on dev. Follow-ups (Profile UI affordance, commodity-only penalty, pin-priority) logged in `docs/next_steps.md`. |
