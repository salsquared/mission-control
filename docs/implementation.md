...
| Cross-cutting | Polish backlog (PB-N) | ◐ | PB-2/3/4/7/9/10/11/12/13 ✅ shipped 2026-05-16 (correctness/UX/doc fixes that were originally tagged RAH-N — see migration table). **Open ⏳:** PB-1 (company-name normalization), PB-5 (Gmail webhook crash recovery), PB-6 (Pub/Sub messageId dedup), PB-8 (Notification dedup constraint), PB-14 (directory→watchlist drift), PB-15 (posting feed filters: type/location/remote). |
...
- ⏳ **PB-14 — Company directory edits don't propagate to existing watchlist rows.** Discovered 2026-05-17 fixing the stale Rocket Lab slug (`rocketlabusa` → `rocketlab`): when a user picks an entry from `lib/company-directory.ts` via "Watch company," the modal POSTs `entry.config` and that JSON gets persisted onto `Watchlist.config`. Future directory corrections (slug typos, switched ATS, renamed careerSite) never reach existing rows — the user gets repeated 404/500 errors on "Run now" until they manually delete + re-add the watchlist. Fix options, cheapest first: (a) one-off `scripts/tests/rocket-lab-slug-check.ts`-style migration scripts when we catch a stale slug — fine for the current ~5 entries; (b) a `directoryKey` column on `Watchlist` that hydrates `config` from `COMPANY_DIRECTORY` at fetch-time so the source of truth stays in code — more correct but a schema migration. Defer to (b) only if the directory grows past ~30 entries or we ship more than one slug correction.
- ⏳ **PB-15 — Filters on the new-postings feed (type, location, etc).** `components/cards/NewPostingsCard.tsx` currently renders postings flat — no way to narrow by "intern only" or "remote only" once a few watchlists are pulling. Two-phase work: (1) **data** — add an `employmentType` column to `JobPosting` (`prisma/schema.prisma`) and extract it in each fetcher. Greenhouse exposes `metadata` + `departments`; Lever has `categories.commitment`; Ashby already has `employmentType` (currently dumped into `snippet`); Workday has no clean field — fall back to title-keyword heuristics (`/\b(intern|internship)\b/i`, `/\b(co-?op)\b/i`, `/\b(senior|principal|staff)\b/i`). LinkedIn HTML cards expose it via `job-search-card__benefits` / `job-search-card__experience-level`. (2) **UI** — filter chips at the top of `NewPostingsCard`: type (full-time / intern / contract / part-time), location (free-text contains, applied client-side against `posting.location`), remote toggle (heuristic: `/(remote|anywhere)/i` in location). Add a `seniority` chip group as stretch (junior / mid / senior, inferred from title). All filters client-side first — no API changes needed beyond returning `employmentType`. Add a tag-state cookie or store on `useAppStore` so the user's filter selection persists per device.
