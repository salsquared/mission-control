...
async function testCategory(category: string) {
    try {
        ...
        rows.each((i, row) => {
            ...
            if (cells.length >= 5) {
                let rank = parseInt($(cells[0]).text().trim(), 10);
                let modelNameNode = $(cells[2]).find('a[title]').first();
                let modelName = modelNameNode.attr('title')?.trim() || modelNameNode.text().trim() || $(cells[2]).text().trim();
                let eloScore = parseInt($(cells[3]).text().trim().replace(/,/g, ''), 10);
                let votes = parseInt($(cells[4]).text().trim().replace(/,/g, ''), 10);
