 ...
 *   which is what keeps a smoke that drives 65 live route handlers hermetic.
...
/** The standard wiring, uniform across all 65 guarded routes today. */
...
//
// Drives EVERY exported HTTP method of all 65 guarded routes. This is also what
...
async function checkAnonymousIs401(): Promise<void> {
    section("4. Behavioural per-route — no verified identity → 401 (all 65 guarded routes, every method)");
    __resetViewer();
    let methodCount = 0;
...
async function checkCrewIs403OnOwnerRoutes(): Promise<void> {
    section("5. Behavioural per-route — authenticated crew → 403:owner-only (all 26 owner-only routes)");
    __seedViewer(CREW_VIEWER);
    try {
        let methodCount = 0;
        for (const spec of MANIFEST) {
