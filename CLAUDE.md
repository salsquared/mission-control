...
2. **Once a callsite migrates to the Lunary registry (LOP-6), iterate via `lunary.renderTemplate(slug, vars)`** — not via inline string edits in code. Pre-cutover: prompts live in `SYSTEM_PROMPT` constants + builder functions; post-cutover: code fetches `{ messages, model, temperature, max_tokens }` from Lunary and only computes the dynamic substitution variables in-process. `docs/llm-prompts/<slug>.md` is the canonical disk snapshot — update there same-day when you edit a template in Lunary's dashboard, so `git log -p` diffs cleanly. The "Migrated to registry?" column in `docs/llm-calls.md` tracks per-callsite rollout.
