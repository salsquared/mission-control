...
**Cross-tier call dedup** (`lib/ai/llm-cache.ts`; design [`docs/archive/cross-tier-llm-dedup.html`](./docs/archive/cross-tier-llm-dedup.html)). dev (:4101/dev.db) and prod (:3101/prod.db) — plus both schedulers — run the same code on one box against the same data, so every cacheable LLM step would call Gemini twice. `chatJSON` and `parseApplicationEmail` route through `llmCached(req, compute)`, which content-hashes the request (resolved model + rendered prompt + output JSON Schema), checks a **shared SQLite file both tiers open** (`data/llm-cache.db`, WAL, gitignored), and either returns a cached result, leads the compute, or follows the other tier's in-flight one (an `INSERT OR IGNORE` reservation gives single-flight even on simultaneous fire — the Gmail push fan-out case). The rate slot (`acquireGeminiSlot`) lives **inside** `compute()`, so only the leader spends a token. Caching is **default-on** via `cache?: boolean` on `ChatJSONOptions`; set `cache: false` for intentionally-generative "suggest/draft" callsites (the 6 opted-out today — see the cache-on/off table in `llm-calls.html`). Content-addressing means a prompt/model change auto-invalidates, so the daily `llm-cache-prune` scheduler job is pure housekeeping. The cache is **best-effort, never load-bearing**: any store failure (better-sqlite3 ABI mismatch after an nvm Node switch, unwritable file) degrades to a direct uncached `compute()`. `better-sqlite3` is a direct dep loaded via a guarded dynamic `import()` (in `serverExternalPackages` — it's a native module). As of the arXiv rate-limit work (2026-05-31) the store + single-flight protocol moved into the generic base `lib/shared-sqlite-cache.ts`; `llm-cache.ts` is now a thin content-addressed adapter over it (see the next subsection).

### Shared cross-tier SQLite cache base + arXiv rate bucket

`lib/shared-sqlite-cache.ts` (`createSharedCache`) is the generic engine behind the cross-tier-dedup pattern (design [`docs/archive/arxiv-rate-limit-fix.html`](./docs/archive/arxiv-rate-limit-fix.html)): a best-effort keyed store on a **third** SQLite file (neither tier's Prisma DB) with a reserve→lead/compute→finish single-flight protocol (follower poll + stale-lease steal). `getOrCompute(key, compute, { ttlSeconds })` — `ttlSeconds` absent ⇒ `expiry=NULL` (content-addressed, never expires: the **LLM cache** adapter `lib/ai/llm-cache.ts`); present ⇒ TTL'd, an expired `done` row is re-led (the **research cache** adapter `lib/research/shared-cache.ts`). It also exports a low-level `openDb()` seam for non-keyed callers.

Two adapters + one wrapper build on it today:
- **Research routes** (`/api/research`, `/api/research/{review,historical,hf}`) wrap with **`withSharedCache()`** (`lib/research/shared-cache.ts`, `data/research-cache.db`) — a sibling of `withCache` (the generic `lib/cache.ts` wrapper is deliberately NOT touched), with its own in-process L1 + stale-fallback, delegating only the upstream fetch to the shared store so dev+prod fetch each arXiv/HF key once instead of twice. `scripts/gen-api-docs.ts` recognizes `withSharedCache` (not just `withCache`) for the cache chip.
- **arXiv rate bucket** (`lib/arxiv/rate-limit.ts:acquireArxivSlot`) reuses the `openDb()` seam for a single-row bucket in `data/arxiv-bucket.db` — ONE shared paced line (`BEGIN IMMEDIATE` consume) across dev+prod+schedulers, since they share one IP and arXiv soft-limits ~1 req/3s. `ARXIV_RATE_PER_MIN` is the **shared** rate (14, not per-process); on a shared-file failure each process falls back to a per-process bucket at HALF that rate. **Recovery / circuit breaker** (Layer 4, 2026-06-02): pacing alone never recovered once arXiv had IP-throttled us — a 429 made the route 500, nothing cached, the next dash load re-poked arXiv, and the throttle window never cleared. A second single-row table in the SAME file (`arxiv_cooldown`) plus a per-process fallback var implements a cross-tier cooldown: **all** arXiv fetches go through `lib/arxiv/fetch.ts:fetchArxivXml` (the only sanctioned arXiv-fetch path — paces, detects a 429 / plaintext "Rate exceeded" 200, returns Atom XML), which calls `noteArxivRateLimited()` to MAX-extend the cooldown; while it's active `acquireArxivSlot()` fast-fails with `ArxivRateLimitCooldownError` (zero arXiv calls, both tiers) so the IP block can clear, then one probe re-arms or recovers. Default `ARXIV_COOLDOWN_MS=900000` (15 min). `fetchArxivXml` throws `ArxivUnavailableError` (`lib/arxiv/errors.ts`; the cooldown error subclasses it); the research routes re-throw it and `withSharedCache`'s `fallbackOnError` serves `[]` (200, **uncached**) instead of a 500 cascade. New arXiv caller: go through `fetchArxivXml`, never `acquireArxivSlot` + bare `fetch`.

All three files (`data/{llm-cache,research-cache,arxiv-bucket}.db`) are gitignored (`*.db` glob), WAL, best-effort (any failure degrades to direct/uncached), and pruned by the daily `llm-cache-prune` scheduler job (which now prunes the research cache too). **Schema-drift self-heal (2026-06-04):** because the cache file outlives the code, `buildInternalStore` calls `reconcileSchema()` on init — it compares the on-disk columns to the expected set (`key, value, status, expiry, reserved_at, done_at`) and, on a mismatch, **drops + rebuilds the table loudly** (the data is throwaway) instead of letting the first prepared statement throw `no such column: …` and silently disable the cache. This is the fix for the bug where the 2026-05-31 `result → value` + `expiry` rename left `data/llm-cache.db` on the old shape, disabling the LLM cache uncached-and-invisible for days until the 2026-06-02 spam exposed the wasted spend (`docs/postmortem-self-notification-mail-loop.html` §5/§9; hermetic `shared-cache-schema-drift-smoke.ts`). So a future column change to `TABLE_COLUMNS` self-corrects on the next start. New cross-tier caller: add an adapter over `createSharedCache` (or reuse `openDb()`), never re-implement the protocol.
...
**Canon generation is the manual exception (2026-05-31).** The scoring pipeline above runs only for url / text / application generation. A **Canon** (role-archetype) instead renders a *hand-curated* selection stored in `Canon.selection` (JSON): `lib/canons/selection.ts:resolveSelection` turns it into the same `ResumeSelection` shape, skipping scoring / spine / one-page-prune entirely. `POST /api/resumes` with a `canonId` routes to a self-contained `generateCanonManualResume` handler (early return — the auto-pipeline is untouched); generation is **manual-only**, so a canon with no `selection` returns a "build one first" error rather than auto-generating the irrelevant picks the redesign retired (OQ15). The selection is edited via `GET`/`PUT /api/canons/[id]/selection` from the builder overlay `components/overlays/ResumeBuilderOverlay.tsx` (opened by CanonsCard's "Edit & generate"); AI rewrite/tagline are opt-in (`options.rewrite` / `options.tagline`, default off → verbatim render). Design archived at [`docs/archive/resume-manual-builder.html`](./docs/archive/resume-manual-builder.html).
...
- **Archetype starters** (copy *instead of* the generic template when the doc is that archetype): `docs/_template.design-doc.html` for a **feature/design doc** — it pre-stubs the fixed section order and the rules that are specific to that archetype (not all docs): **context → design → risks & rollback → Open Questions → Tasks**, with `OQ`-numbered open questions (resolve with the user *before* writing code) and a `≥2`-level-nested Tasks checklist (`P1.2.3`) as the final section. The archetype rules live *embodied in the starter*, not in the generic `_template.html` (a generated reference like `apis.html` or a runbook like `backup-recovery` must NOT carry the design-doc tail order). Add a new `_template.<archetype>.html` when a second archetype earns repeatable structure. *(Mirrors the memory files `feedback-design-doc-structure` + `feedback-design-doc-open-questions`.)*
