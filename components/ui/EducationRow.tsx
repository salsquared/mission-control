...
export const EducationRow: React.FC<EducationRowProps> = ({
    ...
    const updateBullet = (idx: number, next: Bullet) => {
        const arr = [...education.bullets];
        arr[idx] = next;
        onUpdate({ bullets: arr });
    };
    const removeBullet = (idx: number) => {
        onUpdate({ bullets: education.bullets.filter((_, i) => i !== idx) });
    };
    const addBullet = () => {
        const text = newBulletText.trim();
        if (!text) return;
        onUpdate({ bullets: [...education.bullets, makeBullet(text)] });
        setNewBulletText("");
    };
    const handleDraft = async () => {
        ...
        setDraftError(null);
        try {
            const result = await api.profile.bullets.assistFill('education', education.id);
        ...
        <div className="relative bg-white/5 border border-white/5 hover:border-emerald-500/20 rounded-md p-6 transition-colors">
            ...
            <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
                <div>
                    ...
                    <EditableField
                        value={dateToInput(education.startDate)}
                ...
                <div>
                    ...
                    <EditableField
                        value={dateToInput(education.endDate)}
                ...
                {education.bullets.length === 0 ? (
                    <>
                        ...
                        <div className="flex items-center gap-2 px-2 py-0.5">
                            <button
                                ...
                                title="Draft starter bullets with the LLM, grounded on this education's spine fields"
                            >
                    ...
                    education.bullets.map((b, idx) => (
                        <BulletRow
                            ...
                            rewriteContext={{ parentKind: 'education', parentId: education.id }}
                        />
                    ))
                )}
            ...
            <ScratchpadOverlay
                ...
                entityLabel={`${education.degree || "Education"} — ${education.institution || "Institution"}`}
                initialValue={education.scratchpad ?? null}
                onSave={(next) => {
