                                ...
                                {visibleSysLogs.slice(-LOG_DISPLAY_CAP).reverse().map((log) => (
                                    <div key={log.id} className="flex gap-3 w-full border-b border-white/5 pb-1.5 first:border-0 first:pb-0">
                                        <div className="flex gap-2 shrink-0">
                                            ...
                                            <span className={`self-start whitespace-nowrap px-1 rounded text-[10px] leading-[1rem] ${log.source === 'scheduler' ? 'bg-purple-500/20 text-purple-300' : 'bg-yellow-500/20 text-yellow-300'}`}>{log.source === 'scheduler' ? 'SCH' : 'WEB'}</span>
                                        </div>
                        ...
                        <div className="flex flex-col gap-4 p-4 rounded-xl bg-black/20 border border-white/5">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col truncate min-w-0">
                                    <span className="text-sm font-semibold text-slate-200 truncate">{user?.email || "Owner"}</span>
                                    <span className={`text-xs truncate ${googleConnected ? "text-emerald-400" : "text-amber-400"}`}>
